...
export default function Admin() {
  ...
  function hasDateConflict(currentRequest) {
    return bookingRequests.some((request) => {
      ...
      return (
        currentRequest.start_date < request.end_date &&
        currentRequest.end_date > request.start_date
      );
    });
  }
  ...
  async function acceptRequest(request) {
    ...
    const proposedPrice = window.prompt(
      "Tarif proposé au client (€) :",
      String(request.owner_price || request.estimated_total || "")
    );

    if (proposedPrice === null) return;

    const ownerMessage = window.prompt(
      "Message à envoyer au client :",
      "Votre demande est acceptée. La réservation sera confirmée après validation finale et paiement des arrhes."
    );
    ...
    try {
      ...
      const discountAmount =
        Number(request.estimated_total || 0) - Number(proposedPrice || 0);

      const { error } = await supabase
        .from("booking_requests")
        .update({
          status: "accepted",
          owner_price: Number(proposedPrice),
  ...
  async function refuseRequest(request) {
    const ownerMessage = window.prompt(
      "Message de refus à envoyer au client :",
      "Nous sommes désolés, mais les dates demandées ne sont malheureusement pas disponibles."
    );
  ...
  async function confirmRequest(request) {
    if (hasDateConflict(request) && request.status !== "accepted") {
      alert("Impossible de confirmer : une autre réservation est déjà active sur ces dates.");
      return;
    }
    ...
    try {
      await sendDecisionEmail(
        request,
        "confirmed",
        request.owner_price || request.estimated_total,
        ownerMessage
      );
  ...
  async function cancelRequest(request) {
    const confirmed = window.confirm("Annuler cette demande/réservation ?");
    if (!confirmed) return;
  ...
  async function deleteCustomer(customer) {
    const confirmed = window.confirm(
      `Supprimer le client ${customer.first_name || ""} ${customer.last_name || ""} ?`
    );
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (customerSort.key === "booking_count") {
  ...
  return (
    <main style={styles.page}>
      <section style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            Tableau de bord central : demandes, clients, calendrier, CRM et suivi.
          </p>
        </div>

        <div style={styles.headerActions}>
          <button style={styles.refreshButton} onClick={loadAdminData}>
            Actualiser
          </button>
      ...
      <section style={styles.statsGrid}>
        <StatCard label="Demandes" value={stats.requests} />
        <StatCard label="En attente" value={stats.pending} />
        <StatCard label="Acceptées" value={stats.accepted} />
        <StatCard label="Payées / confirmées" value={stats.paid} />
        <StatCard label="Clients" value={stats.customers} />
        <StatCard label="Clients fidèles" value={stats.loyal} />
      </section>
      ...
      {!loading && !error && activeTab === "requests" && (
        <section style={styles.contentGrid}>
              ...
              <div style={styles.list}>
                {filteredRequests.map((request) => (
                  ...
                  >
                    ...
                    <div style={styles.muted}>
                      {request.guest_email || "Email non renseigné"}
                    </div>

                    <div style={styles.price}>
                      {request.owner_price
                        ? `${request.owner_price} € proposés`
                        : request.estimated_total
                        ? `${request.estimated_total} € estimés`
                        : "Prix à confirmer"}
                    </div>
                  </button>
                ))}
          ...
          <div style={styles.panel}>
            <h2 style={styles.panelTitle}>Consultation rapide</h2>

            {!selectedRequest ? (
              <p style={styles.empty}>Sélectionne une demande pour voir le détail.</p>
            ) : (
              <RequestDetail
      ...
      {!loading && !error && activeTab === "customers" && (
        <section style={styles.panel}>
          <div style={styles.panelHeader}>
            ...
            <button style={styles.addButton} onClick={addCustomer}>
              Ajouter client
            </button>
          </div>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <SortableTh label="Créé le" sortKey="created_at" sort={customerSort} onSort={handleCustomerSort} />
                    <SortableTh label="Dernier séjour" sortKey="last_stay" sort={customerSort} onSort={handleCustomerSort} />
                ...
                <tbody>
                  {filteredCustomers.map((customer) => (
                    <tr key={customer.id}>
                      ...
                      <td style={styles.td}>{formatDate(customer.last_stay)}</td>
                      <EditableTd value={customer.notes} onClick={() => updateCustomerField(customer.id, "notes", customer.notes)} />
                      <td style={styles.td}>
                        <button style={styles.deleteButton} onClick={() => deleteCustomer(customer)}>
                          Supprimer
                        </button>
                      </td>
                    </tr>
...
function RequestDetail({ request, onAccept, onRefuse, onConfirm, onCancel }) {
  const status = request.status || "pending";
    ...
    <div style={styles.detail}>
      <div style={styles.detailHeader}>
        <div>
          ...
          <p style={styles.muted}>{request.guest_email}</p>
        </div>
      ...
      <div style={styles.detailGrid}>
        ...
        <Info label="Prix estimatif" value={request.estimated_total ? `${request.estimated_total} €` : "-"} />
        <Info label="Prix proposé" value={request.owner_price ? `${request.owner_price} €` : "-"} />
        <Info label="Réduction" value={request.discount_amount ? `${request.discount_amount} €` : "-"} />
        <Info label="Source" value={request.source || "website"} />
        <Info label="Paiement" value={request.payment_status || "non configuré"} />
      ...
      {request.owner_message && (
        <div style={styles.noteBox}>
          <strong>Dernier message propriétaire</strong>
          <p>{request.owner_message}</p>
        </div>
      )}

      <div style={styles.actions}>
        <button style={styles.acceptButton} onClick={() => onAccept(request)}>
          Accepter avec tarif/message
        </button>

        <button style={styles.refuseButton} onClick={() => onRefuse(request)}>
          Refuser avec message
        </button>

        <button style={styles.confirmButton} onClick={() => onConfirm(request)}>
          Confirmer
        </button>

        <button style={styles.cancelButton} onClick={() => onCancel(request)}>
          Annuler
        </button>
      </div>
    </div>
...
function SortableTh({ label, sortKey, sort, onSort }) {
  ...
  return (
    <th style={styles.th}>
      <button style={styles.thButton} onClick={() => onSort(sortKey)}>
        {label}{arrow}
      </button>
    </th>
  );
...
function EditableTd({ value, onClick }) {
  return (
    <td style={styles.td} onClick={onClick} title="Cliquer pour modifier">
      {value || "-"}
    </td>
  );
}
